...
{
public:
    void TestSolvingVgefConcentrationPde()
    {
        ...
        cells[1]->SetMutationState(p_tip_state);
        cells[2]->SetMutationState(p_tip_state);

        // initial condition for PDE
        cells[0]->GetCellData()->SetItem("vegf_variable",1.0);
        cells[1]->GetCellData()->SetItem("vegf_variable",1.0);
        cells[2]->GetCellData()->SetItem("vegf_variable",1.0);
        ...
        MAKE_PTR_ARGS(VegfEquationPde, p_pde, (cell_population, 1.0, 1.0, 1.0, 0.1, 0.01));
        MAKE_PTR_ARGS(ConstBoundaryCondition<3>, p_bc, (1.0));

        // Create a ChasteCuboid on which to base the finite element mesh used to solve the PDE
        ChastePoint<3> lower(0.0, 0.0, 0.0);
        ChastePoint<3> upper(20.0, 20.0, 20.0);
        ...
        p_pde_modifier->SetDependentVariableName("vegf_variable");
        p_pde_modifier->SetOutputGradient(true);

        p_pde_modifier->SetupSolve(cell_population,"TestVegf");
